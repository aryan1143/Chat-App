...
export const updateProfile = async (req, res) => {
  ...
  try {
    if (!profilePic && !bio && !fullName) {
      return res
        .status(400)
        .json({ message: "Profile data is required to update" });
    }
    ...
    if (profilePic) {
      try {
        ...
        const isDefaultPfp =
          oldProfileURL.split("/").pop().split(".")[0] === "defaultpfp";
        ...
        console.log(
          "Error in update-profile-profile-url-delete auth-controller:",
          error.message,
        );
      }
...
//function to update user preivacy data in DB--------------
export const updatePrivacy = async (req, res) => {
  const { lastSeenAndOnline, readReceipt } = req.body;
  const userId = req.user._id;
  ...
  try {
    ...
    //updating profile bio
    if (typeof lastSeenAndOnline === "boolean")
      updateFeilds.lastSeenAndOnline = lastSeenAndOnline;

    //updating full-name
    if (typeof readReceipt === "boolean")
      updateFeilds.readReceipt = readReceipt;

    await User.findByIdAndUpdate(userId, { $set: updateFeilds });

    res.status(204).json({ message: "Privacy updated succesfully" });
  } catch (error) {
    console.log("Error in update-privacy auth-controller:", error.message);
    res.status(500).json({ message: "Internal server error!" });
  }
};
